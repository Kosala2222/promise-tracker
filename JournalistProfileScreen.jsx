// frontend/components/PoliticianHeader.jsx
...
const API_POLITICIAN = `${API_BASE_URL}/politician`;

const PoliticianHeader = ({ navigation }) => {
  ...
  const loadProfilePhoto = useCallback(async () => {
    try {
      ...
      const res = await axios.get(`${API_POLITICIAN}/profile`);
  ...
  return (
    <View style={styles.container}>
      <TouchableOpacity onPress={() => navigation?.navigate('PoliticianHome')} activeOpacity={0.8}>
      ...
      <TouchableOpacity onPress={() => navigation?.navigate('PoliticianProfile')} activeOpacity={0.8}>
        {photoUri ? (
